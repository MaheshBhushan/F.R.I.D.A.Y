...
def _detection() -> wake.WakeDetection:
    ...
    return wake.WakeDetection(
        ...
        timestamp=0.0,
        turn_id="",
        preroll=b"\x00" * FRAME_BYTES,
        preroll_samples=FRAME_BYTES // 2,
        preroll_seconds=0.02,
        live=live,
    )
...
def _loop(tmp_path, detector, capture, factory, *, follow_up=False):
    ...
    lp = loop_mod.VoiceLoop(
        detector=detector,
        stt_factory=factory,
        llm_transport=brain.FakeLLMTransport([brain.ScriptedTurn(tokens=("Working.",))]),
        capture=capture,
        play_ack=play_ack,
...
def test_delayed_connect_preserves_exact_audio_and_routes_final(tmp_path, monkeypatch):
    async def run():
        ...
        monkeypatch.setattr(stt, "LocalVAD", lambda **_kwargs: CountVAD(16))
...
def test_wake_only_opens_follow_up_and_only_command_routes(tmp_path, monkeypatch):
    async def run():
        ...
        monkeypatch.setattr(stt, "LocalVAD", lambda **_kwargs: CountVAD(1))
        lp, played = _loop(
            tmp_path,
            detector,
            capture,
        ...
        task = asyncio.create_task(lp.handle_detection(_detection(), subscription=initial))
...
def test_single_utterance_wake_prefix_forms_route_once(tmp_path, raw, normalized):
...
def test_friday_stop_during_listening_discards_turn_and_returns_idle(tmp_path):
    transport = ScriptedSTT((_final("Friday stop"),))
    lp, _ = _loop(tmp_path, Detector(), None, lambda: transport)

    async def unexpected(*_args):
        raise AssertionError("hard stop reached the router")

    lp._respond = unexpected
    turn = asyncio.run(lp.handle_detection(_detection()))
    ...
    async def run():
        ...
        events = [
            ...
            async for event in stt.run_utterance(
                ...
                span=span,
                vad=CountVAD(1),
                endpoint_config=stt.EndpointConfig(1, 20, 30, 40),
            )
        ]
...
def test_false_wake_and_second_wake_while_listening_do_not_start_another_turn(tmp_path):
    async def frames():
...
def test_mic_preemption_cancels_and_forgets_in_flight_audio(tmp_path, monkeypatch, stage):
    async def run():
        capture = AudioCaptureService(Source())
        capture._ingest(_frame(0))
        detector = Detector()
        transport = ScriptedSTT(block_enter=stage == "connecting")
        monkeypatch.setattr(stt, "LocalVAD", lambda **_kwargs: CountVAD(100))
        lp, _ = _loop(tmp_path, detector, capture, lambda: transport)
        subscription = capture.subscribe_stt()
        task = asyncio.create_task(lp.handle_detection(_detection(), subscription=subscription))
        lp._turn_task = task
...
def test_connection_failure_cleans_up_and_rearms(tmp_path):
    async def run():
        ...
        lp, _ = _loop(tmp_path, detector, capture, lambda: transport)
        turn = await lp.handle_detection(_detection(), subscription=capture.subscribe_stt())
...
def test_wake_during_tts_is_ignored_without_stopping_speech(tmp_path):
    async def run():
        ...
        assert not old.done()
        assert lp._interrupt_task is not None
        lp._interrupt_task.cancel()
        await asyncio.gather(lp._interrupt_task, return_exceptions=True)
        old.cancel()
...
def test_friday_stop_during_tts_cancels_friday_but_not_external_work(tmp_path):
    async def run():
        capture = AudioCaptureService(Source())
        capture._ingest(_frame(0))
        detector = Detector(wake_on=1)
        interrupt = ScriptedSTT((_final("Friday stop"),), expected_audio=1)
        lp, _ = _loop(tmp_path, detector, capture, lambda: interrupt)
        speaker = Speaker()

        async def active_reasoning():
            await asyncio.Event().wait()

        unrelated = await asyncio.create_subprocess_exec("sleep", "10")
        active = asyncio.create_task(active_reasoning())
        lp._turn_task = active
        lp._speaker = speaker
        lp.audio_state = loop_mod.AudioTurnState.SPEAKING
        ...
        await lp._pump(wake_frame(), asyncio.Event())
        assert lp._interrupt_task is not None
        await asyncio.wait_for(lp._interrupt_task, 1)
        ...
        # Repeating HARD_STOP is harmless and does not touch unrelated work.
        await lp._hard_stop()
        assert lp.audio_state is loop_mod.AudioTurnState.IDLE
        assert unrelated.returncode is None
        unrelated.terminate()
...
def test_hard_stop_during_possible_end_bypasses_patient_deadline():
    async def run():
        queue: asyncio.Queue = asyncio.Queue()
        queue.put_nowait(b"\x01\x00" * (FRAME_BYTES // 2))
        ...
        transport = ScriptedSTT(
            (
                TranscriptEvent("Friday stop", False, turn_event="Update"),
                TranscriptEvent("Friday stop", False, turn_event="Update"),
            ),
            expected_audio=1,
        )
        ...
        seen = [
            ...
            async for event in stt.run_utterance(
                ...
                transport,
                vad=CountVAD(1),
                endpoint_config=stt.EndpointConfig(1, 50, 1500, 2500),
            )
        ]
        assert seen[-1].hard_stop
...
def test_manager_capture_cancellation_drains_all_race_waiters():
    ...
    async def run():
        manager = AudioResourceManager(open_capture=opener, manage_echo_cancel=False)
        stop = asyncio.Event()
...
def test_tts_cleanup_preserves_outer_turn_cancellation(tmp_path):
    ...
    async def run():
        lp = loop_mod.VoiceLoop(
            tts_factory=lambda: tts.FakeSynthesisTransport(sentence_delay=10),
            audio_output=Sink(),
            spans_path=tmp_path / "spans.jsonl",
        )
        task = asyncio.create_task(
            lp._speak_stream(text(), TurnSpan("pending"), loop_mod.Turn("pending"), None)
        )
